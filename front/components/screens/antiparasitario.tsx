...
interface AntiparasitarioProps {
  userName?: string;
  petData?: { 
    name: string; 
    breed: string; 
...
interface AntiparasitarioEvent {
  id: string;
  id_evento?: number;
  tipo: string;
  fecha: string;
...
export default function Antiparasitario({
  ...
  onBack,
  onUpdatePetData,
}: AntiparasitarioProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [showAddEvent, setShowAddEvent] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState<AntiparasitarioEvent | null>(null);
  const [events, setEvents] = useState<AntiparasitarioEvent[]>([]);
  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      try {
        setLoading(true);
        const eventos = await api.eventoSalud.getByAnimal(pet.id_animal, 'antiparasitario');
        const mappedEvents: AntiparasitarioEvent[] = eventos.map((e: any) => {
  ...
  const handleSaveEvent = async () => {
    if (!pet.id_animal) {
      const newEvent: AntiparasitarioEvent = {
  ...
  const handleEventClick = (event: AntiparasitarioEvent) => {
  ...
  const eventsByYear = filteredEvents.reduce((acc, event) => {
    ...
    acc[year].push(event);
    return acc;
  }, {} as { [year: number]: AntiparasitarioEvent[] });
        ...
        <div className="vaccines-container">
          <div className="vaccines-header">
            <div className="vaccines-header-left">
              ...
              <h1 className="vaccines-title">Registrar evento</h1>
            </div>
          </div>
          ...
          <div className="vaccine-form-icon-wrapper">
            <Image src={antiparasitarioIcon} alt="Antiparasitario" width={84} height={84} />
          </div>

          <div className="vaccine-form-container">
            <div className="vaccine-form-field">
              <label className="vaccine-form-label">Tipo de antiparasitario</label>
              <Input
                type="text"
                placeholder="Ej: Desparasitante interno, externo, etc."
            ...
            <div className="vaccine-form-field">
              <label className="vaccine-form-label">Fecha del evento</label>
              <Input
                type="date"
      ...
      <div className="vaccines-container">
        <div className="vaccines-header">
          <div className="vaccines-header-left">
            ...
            <h1 className="vaccines-title">Antiparasitario</h1>
          </div>
        ...
        <div className="vaccines-content">
          ...
          <div className="vaccines-info-banner">
            ...
            <span className="vaccines-info-text">Información sobre antiparasitario</span>
          </div>
          <div className="vaccines-header-line">
            <Image src={lineSvg} alt="Line separator" width={336} height={2} />
          </div>
            ...
            <div className="vaccine-details-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="vaccine-details-content">
                <div className="vaccine-details-field">
                  <span className="vaccine-details-label">Fecha del evento</span>
